...
export function ByteArray(stream, endian) {
  var o = Object.create(null, {
    ...
    readBytes: {
      value: function (buffer, offset, len) {
        ...
        for (; i < len; ++i) dst.setUint8(offset++, src.getUint8(i));
    ...
    readString: {
      value: function (len) {
        ...
        for (; i < len; ++i) text += String.fromCharCode(src.getUint8(i));
...
export function getTransportAudio() {
  var ctx = null;
  if (typeof AudioContext !== "undefined") {
    ctx = new AudioContext();
  } else {
    alert(
      "Web Audio API does not appear to be supported. Use Chrome, Safari or Firefox"
    );
  }
  return ctx;
}
export function CorePlayer() {
  ...
  if (!window.neoart.audioContext) {
    window.neoart.audioContext = getTransportAudio();
  }
